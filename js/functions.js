...
(function ($) {
    ...
    AP.core = {
        functions: function () {
            AP.core.pageLoader();
            AP.core.responsiveClasses();
            AP.core.goToTop();

            AP.core.screenSizeControl();
            AP.core.rtlStatus();
            AP.core.customHeight();
            AP.core.equalize();
            AP.core.stickyFooter();
    ...
    AP.header = {
        functions: function () {
            AP.header.logoStatus();
            AP.header.stickyHeader();
            AP.header.topBar();
            AP.header.topSearch();
            AP.header.mainMenu();
            AP.header.pageTitle();
...
*/
        ...
        dotsMenu: function () {
            if ($dotsMenu.length > 0) {
                ...
                $dotsMenu.find('a[href="#' + AP.header.currentSection() + '"]').parent("li").addClass('current');
            }
        },
        onepageMenu: function () {
            if ($mainmenu.hasClass("menu-one-page")) {
                $mainmenu.find("nav > ul > li > a").parents("li").removeClass('current');
                $mainmenu.find('nav > ul > li > a[href="#' + AP.header.currentSection() + '"]').parent("li").addClass('current');
            }
        },
    ...
    AP.slider = {
        functions: function () {
            AP.slider.sliderScreenSizeControl();
            AP.slider.APSlider();
            AP.slider.carousel();
        },
        sliderScreenSizeControl: function () {
            if ($(".AP-slider").length > 0) {
                var headerHeight = $header.height(),
                    ...
                    sliderTargetElements = $(".AP-slider, .AP-slider .owl-stage-outer, .AP-slider .owl-stage, .AP-slider .slide"),
                    customHeight = $(".AP-slider").data("height"),
                    responsiveHeightxs = $(".AP-slider").data("height-xs") || 300;
                    ...
                    if (transparentHeader) {
                        if (sliderFullscreen) {
                            ...
                            if (!$header.hasClass(".header-transparent")) {
                                $(".AP-slider .slide").css('padding-top', screenHeightExtra + 'px');
        ...
        APSlider: function () {
            if (!$().owlCarousel) {
                console.log('APSlider: owlCarousel plugin is missing.');
                return true;
            }
            if ($(".AP-slider").length > 0) {
                $(".AP-slider").each(function () {
                    var elem = $(this),
    ...
    AP.elements = {
        functions: function () {
            AP.elements.naTo();
            AP.elements.textRotator();
            AP.elements.buttons();
            AP.elements.accordion();
            AP.elements.tabs();
            AP.elements.animations();
            AP.elements.parallax();
            AP.elements.responsiveVideos();
            AP.elements.counters();
            AP.elements.countdownTimer();
            AP.elements.progressBar();
            AP.elements.pieChart();
            AP.elements.maps();
            AP.elements.gridLayout();
            AP.elements.tooltip();
            AP.elements.popover();
            AP.elements.lightBoxAP();
            AP.elements.youTubeBgPlayer();
            AP.elements.modal();
            AP.elements.pinOnScroll();
            AP.elements.other();
        },
            ...
            if ($gridLayout.length > 0) {
                $gridLayout.each(function () {
                    ...
                    if (elem.next().hasClass("infinite-scroll")) {
                        AP.elements.gridLayoutInfinite(elem, gridItem, gridMargin);
                    }
        ...
        lightBoxAP: function () {
            if (!$().magnificPopup) {
                console.log('lightBoxAP: magnificPopup plugin is missing.');
                return true;
            }
    ...
    AP.widgets = {
        functions: function () {
            AP.widgets.twitter();
            AP.widgets.flickr();
            AP.widgets.contactForm();
            AP.widgets.subscribeForm();
        },
        twitter: function () {
    ...
    $window.on('resize', AP.documentOnResize.functions);
    ...
    AP.documentReady = {
        functions: function () {
            ...
            AP.header.functions();
            AP.slider.functions();
            AP.elements.functions();
            AP.widgets.functions();
        },
    };
    $(document).ready(AP.documentReady.functions);
    ...
    $(window).on('load', AP.documentOnLoad.functions);
    ...
    $window.on('scroll', AP.documentScroll.functions);



    AP.lightBoxAP = function () {
